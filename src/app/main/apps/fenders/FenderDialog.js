...
function FenderDialog(props)
{
   const dispatch = useDispatch();
   const fenderDialog = useSelector(({fendersApp}) => fendersApp.fenders.fenderDialog);
   const vehicleTypes = useSelector(({fendersApp}) => fendersApp.fenders.vehicleTypes);
   ...
   const initDialog = useCallback(
      () => {
         if ( fenderDialog.type === 'edit' && fenderDialog.data )
         {
            setForm({...fenderDialog.data});
         }

         if ( fenderDialog.type === 'new' )
         {
            setForm({
               ...defaultFormState,
               ...fenderDialog.data,
      ...
      [fenderDialog.data, fenderDialog.type, setForm]
   );
   ...
   useEffect(() => {
      ...
      * After Dialog Open
      */
      if ( fenderDialog.props.open ) {
         initDialog();
      }

   }, [fenderDialog.props.open, initDialog]);

   function closeComposeDialog()
   {
      fenderDialog.type === 'edit' ? dispatch(Actions.closeEditFenderDialog()) : dispatch(Actions.closeNewFenderDialog());
   }
   ...
   function handleSubmit(event)
   {
      ...
      if ( fenderDialog.type === 'new' ) {
         dispatch(Actions.addFender(form));
      } else {
         dispatch(Actions.updateFender(form));
      }
   ...
   {
      dispatch(Actions.removeFender(form._id));
      closeComposeDialog();
   }
         ...
         <AppBar position="static" elevation={1}>
            <Toolbar className="flex w-full">
               <Typography variant="subtitle1" color="inherit">
                  {fenderDialog.type === 'new' ? 'New Fender' : 'Edit Fender'}
               </Typography>
            </Toolbar>
            <div className="flex flex-col items-center justify-center pb-24">
               {fenderDialog.type === 'edit' && (
                  <>
