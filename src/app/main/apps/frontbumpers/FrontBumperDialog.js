...
function FrontBumperDialog(props)
{
   const dispatch = useDispatch();
   const frontbumperDialog = useSelector(({frontbumpersApp}) => frontbumpersApp.frontbumpers.frontbumperDialog);
   const vehicleTypes = useSelector(({frontbumpersApp}) => frontbumpersApp.frontbumpers.vehicleTypes);
   const classes = useStyles(props);
   const {form, handleChange, setForm} = useForm(defaultFormState);

   const initDialog = useCallback(
      () => {
         if ( frontbumperDialog.type === 'edit' && frontbumperDialog.data )
         {
            setForm({...frontbumperDialog.data});
         }

         if ( frontbumperDialog.type === 'new' )
         {
            setForm({
               ...defaultFormState,
               ...frontbumperDialog.data,
               id: FuseUtils.generateGUID()
            });
         }
      },
      [frontbumperDialog.data, frontbumperDialog.type, setForm]
   );
   ...
   useEffect(() => {
      ...
      * After Dialog Open
      */
      if ( frontbumperDialog.props.open ) {
         initDialog();
      }

   }, [frontbumperDialog.props.open, initDialog]);

   function closeComposeDialog()
   {
      frontbumperDialog.type === 'edit' ? dispatch(Actions.closeEditFrontBumperDialog()) : dispatch(Actions.closeNewFrontBumperDialog());
   }
   ...
   function handleSubmit(event)
   {
      ...
      if ( frontbumperDialog.type === 'new' ) {
         dispatch(Actions.addFrontBumper(form));
      } else {
         dispatch(Actions.updateFrontBumper(form));
      }
   ...
   {
      dispatch(Actions.removeFrontBumper(form._id));
      closeComposeDialog();
   }
         ...
         <AppBar position="static" elevation={1}>
            <Toolbar className="flex w-full">
               <Typography variant="subtitle1" color="inherit">
                  {frontbumperDialog.type === 'new' ? 'New Front Bumper' : 'Edit Front Bumper'}
               </Typography>
            </Toolbar>
            <div className="flex flex-col items-center justify-center pb-24">
               {frontbumperDialog.type === 'edit' && (
                  <>
...
export default FrontBumperDialog;
