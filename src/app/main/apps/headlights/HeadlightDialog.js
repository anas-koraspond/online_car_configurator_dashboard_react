...
function HeadlightDialog(props)
{
   const dispatch = useDispatch();
   const headlightDialog = useSelector(({headlightsApp}) => headlightsApp.headlights.headlightDialog);
   const vehicleTypes = useSelector(({headlightsApp}) => headlightsApp.headlights.vehicleTypes);
   const classes = useStyles(props);
   const {form, handleChange, setForm} = useForm(defaultFormState);

   const initDialog = useCallback(
      () => {
         if ( headlightDialog.type === 'edit' && headlightDialog.data )
         {
            setForm({...headlightDialog.data});
         }

         if ( headlightDialog.type === 'new' )
         {
            setForm({
               ...defaultFormState,
               ...headlightDialog.data,
               id: FuseUtils.generateGUID()
            });
         }
      },
      [headlightDialog.data, headlightDialog.type, setForm]
   );
   ...
   useEffect(() => {
      ...
      * After Dialog Open
      */
      if ( headlightDialog.props.open ) {
         initDialog();
      }

   }, [headlightDialog.props.open, initDialog]);

   function closeComposeDialog()
   {
      headlightDialog.type === 'edit' ? dispatch(Actions.closeEditHeadlightDialog()) : dispatch(Actions.closeNewHeadlightDialog());
   }
   ...
   function handleSubmit(event)
   {
      ...
      if ( headlightDialog.type === 'new' ) {
         dispatch(Actions.addHeadlight(form));
      } else {
         dispatch(Actions.updateHeadlight(form));
      }
   ...
   {
      dispatch(Actions.removeHeadlight(form._id));
      closeComposeDialog();
   }
         ...
         <AppBar position="static" elevation={1}>
            <Toolbar className="flex w-full">
               <Typography variant="subtitle1" color="inherit">
                  {headlightDialog.type === 'new' ? 'New Headlight' : 'Edit Headlight'}
               </Typography>
            </Toolbar>
            <div className="flex flex-col items-center justify-center pb-24">
               {headlightDialog.type === 'edit' && (
                  <>
...
export default HeadlightDialog;
