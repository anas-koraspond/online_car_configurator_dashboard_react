...
function HitchDialog(props)
{
   const dispatch = useDispatch();
   const hitchDialog = useSelector(({hitchsApp}) => hitchsApp.hitchs.hitchDialog);
   ...
   const initDialog = useCallback(
      () => {
         if ( hitchDialog.type === 'edit' && hitchDialog.data )
         {
            setForm({...hitchDialog.data});
         }

         if ( hitchDialog.type === 'new' )
         {
            setForm({
               ...defaultFormState,
               ...hitchDialog.data,
      ...
      [hitchDialog.data, hitchDialog.type, setForm]
   );
   ...
   useEffect(() => {
      ...
      * After Dialog Open
      */
      if ( hitchDialog.props.open ) {
         initDialog();
      }

   }, [hitchDialog.props.open, initDialog]);

   function closeComposeDialog()
   {
      hitchDialog.type === 'edit' ? dispatch(Actions.closeEditHitchDialog()) : dispatch(Actions.closeNewHitchDialog());
   }
   ...
   function handleSubmit(event)
   {
      ...
      if ( hitchDialog.type === 'new' ) {
         dispatch(Actions.addHitch(form));
      } else {
         dispatch(Actions.updateHitch(form));
      }
   ...
   {
      dispatch(Actions.removeHitch(form._id));
      closeComposeDialog();
   }
         ...
         <AppBar position="static" elevation={1}>
            <Toolbar className="flex w-full">
               <Typography variant="subtitle1" color="inherit">
                  {hitchDialog.type === 'new' ? 'New Hitch' : 'Edit Hitch'}
               </Typography>
            </Toolbar>
