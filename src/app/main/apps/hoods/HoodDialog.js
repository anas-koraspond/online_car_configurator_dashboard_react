...
function HoodDialog(props)
{
   const dispatch = useDispatch();
   const hoodDialog = useSelector(({hoodsApp}) => hoodsApp.hoods.hoodDialog);
   const vehicleTypes = useSelector(({hoodsApp}) => hoodsApp.hoods.vehicleTypes);
   ...
   const initDialog = useCallback(
      () => {
         if ( hoodDialog.type === 'edit' && hoodDialog.data )
         {
            setForm({...hoodDialog.data});
         }

         if ( hoodDialog.type === 'new' )
         {
            setForm({
               ...defaultFormState,
               ...hoodDialog.data,
      ...
      [hoodDialog.data, hoodDialog.type, setForm]
   );
   ...
   useEffect(() => {
      ...
      * After Dialog Open
      */
      if ( hoodDialog.props.open ) {
         initDialog();
      }

   }, [hoodDialog.props.open, initDialog]);

   function closeComposeDialog()
   {
      hoodDialog.type === 'edit' ? dispatch(Actions.closeEditHoodDialog()) : dispatch(Actions.closeNewHoodDialog());
   }
   ...
   function handleSubmit(event)
   {
      ...
      if ( hoodDialog.type === 'new' ) {
         dispatch(Actions.addHood(form));
      } else {
         dispatch(Actions.updateHood(form));
      }
   ...
   {
      dispatch(Actions.removeHood(form._id));
      closeComposeDialog();
   }
         ...
         <AppBar position="static" elevation={1}>
            <Toolbar className="flex w-full">
               <Typography variant="subtitle1" color="inherit">
                  {hoodDialog.type === 'new' ? 'New Hood' : 'Edit Hood'}
               </Typography>
            </Toolbar>
            <div className="flex flex-col items-center justify-center pb-24">
               {hoodDialog.type === 'edit' && (
                  <>
