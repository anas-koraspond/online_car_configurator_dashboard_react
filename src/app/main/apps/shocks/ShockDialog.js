...
function ShockDialog(props)
{
   const dispatch = useDispatch();
   const shockDialog = useSelector(({shocksApp}) => shocksApp.shocks.shockDialog);
   const vehicleTypes = useSelector(({shocksApp}) => shocksApp.shocks.vehicleTypes);
   ...
   const initDialog = useCallback(
      () => {
         if ( shockDialog.type === 'edit' && shockDialog.data )
         {
            setForm({...shockDialog.data});
         }

         if ( shockDialog.type === 'new' )
         {
            setForm({
               ...defaultFormState,
               ...shockDialog.data,
      ...
      [shockDialog.data, shockDialog.type, setForm]
   );
   ...
   useEffect(() => {
      ...
      * After Dialog Open
      */
      if ( shockDialog.props.open ) {
         initDialog();
      }

   }, [shockDialog.props.open, initDialog]);

   function closeComposeDialog()
   {
      shockDialog.type === 'edit' ? dispatch(Actions.closeEditShockDialog()) : dispatch(Actions.closeNewShockDialog());
   }
   ...
   function handleSubmit(event)
   {
      ...
      if ( shockDialog.type === 'new' ) {
         dispatch(Actions.addShock(form));
      } else {
         dispatch(Actions.updateShock(form));
      }
   ...
   {
      dispatch(Actions.removeShock(form._id));
      closeComposeDialog();
   }
         ...
         <AppBar position="static" elevation={1}>
            <Toolbar className="flex w-full">
               <Typography variant="subtitle1" color="inherit">
                  {shockDialog.type === 'new' ? 'New Shock' : 'Edit Shock'}
               </Typography>
            </Toolbar>
            <div className="flex flex-col items-center justify-center pb-24">
               {shockDialog.type === 'edit' && (
                  <>
